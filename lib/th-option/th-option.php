...
if ($hook === 'appearance_page_amaz_store_thunk_started'  ) {
wp_enqueue_style( 'thunk-started-css', get_template_directory_uri() . '/lib/th-option/assets/css/started.css' );
wp_enqueue_script('amaz-store-admin-load', get_template_directory_uri() . '/lib/th-option/assets/js/th-options.js',array( 'jquery', 'updates' ),'1', true);
...
$data = apply_filters(
    ...
    wp_localize_script( 'amaz-store-admin-load', 'THAdmin', $data); 
...
function tab_constant(){
    ...
    $tab_array = array();
    $tab_array['header'] = array('theme_brand' => __('ThemeHunk','amaz-store'),
    'theme_brand_url' => esc_url($theme_data->get( 'AuthorURI' )),
    'welcome'=>sprintf(esc_html__('Welcome To %1s Theme', 'amaz-store'), esc_html($theme_data->get( 'Name' ),'amaz-store'), $theme_data->get( 'Version' ) ),
    'welcome_desc' => esc_html($theme_data->get( 'Name' ).' is a Free WooCommerce theme for creating clean and professional shopping stores.', 'amaz-store' ),
...
function default_home() {
    ...
    if ( ! current_user_can( 'edit_theme_options' ) ) {
        ...
        wp_send_json_error(
            array(
                'message' => esc_html__( 'You are not allowed to perform this action.', 'amaz-store' ),
            ),
            403
        );
    }
...
if(empty($pages)){
      $post_id = wp_insert_post(array (
       'post_type' => 'page',
       'post_title' => __('Home Page','amaz-store'),
       'post_content' => '',
       'post_status' => 'publish',
    ...
    check_ajax_referer( 'th_admin_nonce', 'nonce' );
      ...
      if ( ! current_user_can( 'install_plugins' ) || ! isset( $_POST['init'] ) || ! $_POST['init'] ) {
        wp_send_json_error(
          array(
            'success' => false,
            'message' => __( 'No plugin specified', 'amaz-store' ),
          )
        );
      }
      ...
      wp_send_json_success(
        array(
          'success' => true,
          'message' => __( 'Plugin Successfully Activated', 'amaz-store' ),
        )
      );
...
function plugin_install_button($plugin){
            ...
            <h4>'.esc_html( $plugin['plugin_name'] ). ' </h4><a class="plugin-detail thickbox open-plugin-details-modal" href="'.esc_url( $plugin['detail_link'] ).'">'.esc_html__( 'Details & Version', 'amaz-store' ).'</a>
            </div>';
        ...
        foreach($recommend_plugins[0] as $slug=>$plugin){
            ...
            $button_class = 'install-now button '.$slug;
             ...
             if ( is_plugin_active( $plugin_init ) ) {
                   ...
                   $button_txt = esc_html__( 'Activated', 'amaz-store' );
                   $detail_link = $install_url = '';
                }

            if ( ! is_plugin_active( $plugin_init ) ){
                    $button_txt = esc_html__( 'Install Now', 'amaz-store' );
                    if ( ! $status ) {
                        ...
                        $button_txt = esc_html__( 'Activate Now', 'amaz-store' );
                    }
                }
...
$boj = new Amaz_Store_theme_option(); ?>
