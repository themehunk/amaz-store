	 ...
	 */
		theme_conrols.on(
			'click', '.customizer-repeater-new-field', function () {
				...
				//Limit repeater starts
			if (!amaz_store_repeater.limit_repeater) {
				var _repeater_count = '';
	 			_repeater_count = parseInt(jQuery( this ).find('input[placeholder="Add new Slide"]').val());
				var _button_to_limit = jQuery( this ).closest('li.customize-control.customize-control-text').find('.customizer-repeater-new-field');
				_repeater_count++;
				checkcounter(_button_to_limit,_repeater_count);
			}
				//Limit repeater End
				return false;
			}
		...
		//Limit repeater
		function checkcounter(buttonlimit,_repeater_counter){
			if (_repeater_counter >= 2) {
				buttonlimit.prop('disabled', true);
			}
		}
		theme_conrols.on(
